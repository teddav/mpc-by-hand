...
# Notation and mathematical context:
# The KOS15 paper uses a specific notation to distinguish between two types of operations
# on the same data:
#
# 1. Vector notation ((F₂)^k):
#    - vectors of k bits (e.g., tⱼ, qⱼ).
#    - notation in paper: '*' for component-wise product, '+' for component-wise XOR.
#    - used in step 0 (IKNP setup).
#
# 2. Field notation (F_{2^k}):
#    - single elements of the large finite field (e.g., Δ, χⱼ).
#    - notation in paper: '·' for finite field multiplication, '+' for field addition.
#    - used in steps 1-3 (correlation check).
...
# The sender's secret Δ is a field element for later security.
delta = F_2k.random_element()
# For this step only, we need its vector representation.
# This corresponds to the paper's notation Δ ∈ (F₂)^k when used with '*' operations.
...
# qj = tj + xj * delta (component-wise product `xⱼ * Δ` from the paper notation)
Q = matrix(
    F2, [T.row(i) + X.row(i).pairwise_product(delta_vec) for i in range(l_extended)]
)
...
chi = [F_2k.random_element() for _ in range(l_extended)]

# Step 2: compute the weighted sums
# Those are done in the field notation (F_{2^k}) to ensure the security of the check.
# In the paper is use the finite field multiplication ('·' in paper notation)
# Convert the matrices to field elements
# Bold letters are used in the paper for vectors t_i and q_i, hence are vectors
T_field = [F_2k(T.row(i)) for i in range(l_extended)]
Q_field = [F_2k(Q.row(i)) for i in range(l_extended)]
# Lower case letter is used in the paper hence x_i are elements of F2
x_field = [F_2k(bit) for bit in x]

# Compute the weighted sums using field arithmetic
x_weighted = sum(x_field[i] * chi[i] for i in range(l_extended))
t_weighted = sum(T_field[i] * chi[i] for i in range(l_extended))
q_weighted = sum(Q_field[i] * chi[i] for i in range(l_extended))

# Step 3 assert t = q + x · Δ is checked using field arithmetic.
